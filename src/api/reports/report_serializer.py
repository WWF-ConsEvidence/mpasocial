...
class ReportSerializer(object):
    ...
    def get_fields(self):
        # Inheritors should either define `fields` or `Meta.model` to generate automatically
        if self.fields is None:
            if hasattr(self, "Meta") and hasattr(self.Meta, "model"):
                exclude_csv_fields = [f.name for f in BaseModel._meta.fields]
                fields = [
                    f.name
                    for f in self.Meta.model._meta.fields
                    if f.name not in exclude_csv_fields
                ]
                self.fields = [ReportField(f, f) for f in fields]
            else:
